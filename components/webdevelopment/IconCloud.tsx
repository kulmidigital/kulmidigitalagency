...
const IconCloud = () => {
  return (
    <div className='p-8 bg-gradient-to-br from-blue-100/50 to-purple-50/50 dark:from-blue-900/30 dark:to-purple-900/30 rounded-2xl'>
      <div className='grid grid-cols-2 gap-8'>
        {icons.map((icon, index) => (
          <div
            key={index}
            className='flex flex-col items-center justify-center p-6 rounded-xl hover:bg-white/50 dark:hover:bg-gray-800/50 transition-all duration-300'>
            {icon.href ? (
              <Link href={icon.href} className='flex flex-col items-center'>
                <icon.Icon className='w-16 h-16 text-gray-700 dark:text-gray-300 hover:text-[#F56E0F] transition-colors mb-3' />
                <span className='text-base text-gray-600 dark:text-gray-300 text-center font-medium hover:text-[#F56E0F]'>
                  {icon.name}
                </span>
              </Link>
            ) : (
