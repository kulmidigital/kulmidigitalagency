...
const Blog = () => {
  ...
  return (
    <section
      className={`${clashDisplay.className} px-4 pt-8 md:px-20 dark:bg-gray-900`}>
      ...
      <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8'>
        {currentPosts.map(({ id, date, title, image, categoryName }, index) => (
          <SlideReveal key={id} direction='up' duration={0.7}>
            <div className='bg-white dark:bg-gray-800 rounded-2xl shadow-md overflow-hidden flex flex-col h-full'>
              {image && (
              ...
              <div className='p-6 flex flex-col flex-grow'>
                <h2
                  className={`${clashDisplay.className} text-xl font-semibold mb-2 dark:text-gray-100`}>
                  {title}
                </h2>
                <div className='flex justify-between items-center mb-4'>
                  <span
                    className={`${plusJakartaSans.className} text-sm text-gray-500 dark:text-gray-400`}>
                    {date}
                  </span>
                  {categoryName && (
                    <span className='text-sm bg-gray-100 dark:bg-gray-700 dark:text-gray-300 px-2 py-1 rounded-full'>
                      {categoryName}
                    </span>
                  )}
                </div>
