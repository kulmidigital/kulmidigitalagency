...
const Pricing = () => {
  ...
  return (
    <div className={`${clashDisplay.className} py-4 md:py-8 px-0 md:px-8 bg-gray-50`}>
      <div className='w-[96%] md:container mx-auto'>
        <SlideReveal direction='up' duration={0.7}>
          <div className='text-center mb-8 sm:mb-12'>
            ...
            <p className={`${plusJakartaSans.className} text-gray-600 text-sm sm:text-base mb-6 sm:mb-8`}>
              Simple pricing. No hidden fees. No contract - cancel any time.
            </p>
            
            <div className='flex items-center justify-center gap-2 sm:gap-4'>
              <span className={`${isAnnual ? 'text-gray-500' : 'text-gray-900'} text-sm sm:text-base`}>Monthly</span>
              <Switch
                checked={isAnnual}
                onCheckedChange={setIsAnnual}
                className="data-[state=checked]:bg-[#F56E0F] bg-gray-200 h-[30px] sm:h-[38px] w-[60px] sm:w-[74px] px-[2px]"
              />
              <span className={`${isAnnual ? 'text-gray-900' : 'text-gray-500'} flex items-center gap-1 sm:gap-2 text-sm sm:text-base`}>
                Annual
          ...
          <div className='grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-8'>
            {currentPlans.map((plan, index) => (
              <div
                key={plan.name}
                className={`relative rounded-2xl sm:rounded-3xl bg-white p-4 sm:p-8 shadow-lg border ${
                  plan.popular ? 'border-[#F56E0F]' : 'border-gray-100'
                } flex flex-col`}
              >
                {plan.popular && (
                ...
                <div className="flex-grow">
                  ...
                  <p className={`${plusJakartaSans.className} text-gray-600 text-sm sm:text-base mb-4 sm:mb-6`}>
                    {plan.description}
                  </p>
                  <div className='mb-4 sm:mb-6'>
                    <span className='text-3xl sm:text-4xl font-bold'>${plan.price}</span>
                    <span className='text-gray-600 text-sm sm:text-base'>/{plan.period}</span>
                    <p className='text-xs sm:text-sm text-gray-500 mt-1'>VAT excluded</p>
                  </div>
                  <ul className='space-y-3 sm:space-y-4 mb-6 sm:mb-8'>
                    {plan.features.map((feature, i) => (
                      <li key={i} className='flex items-start gap-2'>
                        <Check className='w-4 h-4 sm:w-5 sm:h-5 text-[#F56E0F] mt-0.5 flex-shrink-0' />
                        <span className={`${plusJakartaSans.className} text-gray-600 text-sm sm:text-base`}>
                          {feature}
                        </span>
                      </li>
                    ))}
